...
def test_conserva_el_access_log_de_endpoints_de_negocio():
    assert SinRuidoDeHealthCheck().filter(_record("/cotizaciones")) is True
...
def test_sanear_para_log_quita_saltos_de_linea():
    valor = "abc123\r\nINFO:cotizacion.api:cuenta admin creada"
    assert sanear_para_log(valor) == "abc123INFO:cotizacion.api:cuenta admin creada"
